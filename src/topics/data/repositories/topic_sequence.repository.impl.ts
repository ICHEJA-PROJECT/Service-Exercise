...
export class TopicSequenceRepositoryImpl implements TopicSequenceRepository {
    constructor(
        @InjectRepository(TopicSequenceEntity) private readonly topicSequenceRepository: Repository<TopicSequenceEntity>,
    ...
    async findAll(): Promise<TopicSequenceI[]> {
        try {
            const sequences = await this.topicSequenceRepository.find({relations: ["currentTopic", "nextTopic", "learningPath"]});
            return sequences;
        } catch (error) {
            throw new InternalServerErrorException(error);
        }
    ...
    async findByLearningPath(learningPathId: number): Promise<TopicSequenceI[]> {
        try {
            ...
            throw new RpcException({
                message: error.message || "Error al conseguir la secuencia de temas por ruta de aprendizaje",
