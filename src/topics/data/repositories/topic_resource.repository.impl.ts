...
export class TopicResourceRepositoryImpl implements TopicResourceRepository {
    constructor(
        @InjectRepository(TopicResourceEntity) private readonly topicResourceRepository: Repository<TopicResourceEntity>,
    ...
    async create(createTopicResourceDto: CreateTopicResourceDto): Promise<TopicResourceI> {
        try {
            const topic = await this.topicRepository.findOne({where: { id: createTopicResourceDto.topicId}});
            if(!topic) throw new NotFoundException("El tema seleccionado no existe.");

            const resource = await this.resourceRepository.findOne({where: {id: createTopicResourceDto.resourceId}});
            if(!resource) throw new NotFoundException("El recurso seleccionado no existe");
